...
pub fn fs_lib_functions(
    ...
    match name {
        ...
        "write" => {
            ...
            let contents = args[1].compile(ctx, state, output, None, false, true).unwrap_id();
            state.free_reg(filepath);
            state.free_reg(contents);
            output.push(Instr::CallLibFuncVoid(LibFuncVoid::FsWrite, filepath, contents));
            state.add_to_src(ctx, output, span);
        }
        "append" => {
            check_args(args, 2, name, span, state.sources, ctx.file_idx);
            ...
            state.free_reg(filepath);
            state.free_reg(contents);
            output.push(Instr::CallLibFuncVoid(LibFuncVoid::FsAppend, filepath, contents));
            state.add_to_src(ctx, output, span);
        }
        "delete" => {
            check_args(args, 1, name, span, state.sources, ctx.file_idx);
